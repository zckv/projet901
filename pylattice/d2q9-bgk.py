...
def main():
    ...
    av_vels = compute(
        cells, obstacles, param["maxIters"], nx, ny, density, accel, omega
    )
...
@njit(
    float64(float64[:, :, :], boolean[:, :], int32, int32, int32, int32), parallel=True
)
def av_velocity(
    cells,
    obstacles,
    ...
    """
    tot_cells = ((end_blk - start_blk + 1) * obstacles.shape[1]) - obstacles[
        start_blk : end_blk + 1
    ].sum()
    tot_u = 0.0
...
@njit(
    float64(float64[:, :, :], boolean[:, :], int32, int32, float64, int32),
    parallel=False,
)
...
def timestep(cells, obstacles, start_blk, end_blk, nx, ny, density, accel, omega):
    """
    One step elapse
...
@njit(
    float64[:](
        float64[:, :, :], boolean[:, :], int32, int32, int32, float64, float64, float64
    ),
    parallel=False,
)
def compute(cells, obstacles, n, nx, ny, density, accel, omega):
    ...
    for tt in range(n):
        # av_vels[tt] =
        x = timestep(
            cells, obstacles, start_blk, end_blk, nx, ny, density, accel, omega
        )
